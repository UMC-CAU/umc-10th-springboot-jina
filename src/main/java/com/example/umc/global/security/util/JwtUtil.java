...
@Component
public class JwtUtil { // 토큰 발급/해독 기계, JWT 토큰 관련 작업을 하는 유틸
    ...
    /** 토큰에서 소셜 로그인 타입 가져오기
     *
     * @param token 소셜 로그인 타입을 추출할 토큰
     * @return 유저 소셜 로그인 타입을 토큰에서 추출합니다
     */
    ...
    /** 토큰에서 소셜 UID 가져오기
     *
     * @param token 소셜 UID를 추출할 토큰
     * @return 유저 소셜 UID를 토큰 subject에서 추출합니다
     */
    ...
    private String createToken(AuthMember member, Duration expiration) {
        ...
        return Jwts.builder()
                .subject(member.getUsername()) // User UID을 Subject로
